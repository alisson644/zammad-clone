...
RSpec.describe 'Channel::EmailParser#process_failed_articles', aggregate_failures: true, type: :model do
  context 'when receiving unprocessable article' do
    before do
      allow_any_instance_of(HtmlSanitizer::Strict).to receive(:run_sanatization).and_raise(Timeout::Error,
                                                                                           HtmlSanitizer::UNPROCESSABLE_HTML_MSG)
    ...
    it 'does reprocess the unprocessable article' do
      ...
      allow_any_instance_of(HtmlSanitizer::Strict).to receive(:run_sanatization).and_call_original
