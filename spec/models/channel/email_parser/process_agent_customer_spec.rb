...
RSpec.describe 'Channel::EmailParse process email from agent-customer', type: :model do
  let(:group)       { Group.lookup(name: 'Users') }
  let(:other_group) { create(:group) }
  let(:email)       { Channel::EmailParse.new.process({ group_id: group.id, trusted: false }, data) }
  let(:ticket)      { email[0] }
  let(:article)     { email[1] }
  let(:data) do
    <<~MAIL
  ...
  context 'when user is agent and customer' do
    context 'when user is agent in the selected group' do
      ...
      it 'sets articles sender to agent' do
        expect(article.sender.name).to match('Agent')
      end
    end
