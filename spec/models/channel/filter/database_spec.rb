...
RSpec.describe Channel::Filter::Database, type: :channel_filter do
  ...
  describe '.filter_matches?' do
    ...
    context "with operator 'contains'" do
      ...
      context 'with matching upcase string' do
        let(:value) { 'A' }
    ...
    context "with operator 'contains not'" do
      ...
      context 'with matiching upcase string' do
        let(:value) { 'A' }
    ...
    context "with operator 'is any of'" do
      ...
      context 'with matching strng' do
    ...
    context "with operator 'start with one of'" do
      let(:operator) { 'start with one of' }
      ...
      context 'with matching upcase string' do
        let(:value) { ['Daffy.Ducck', 'Elmer.Fudd'] }

        include_examples 'the filter matches'
      end

      context 'witn non-matching string' do
        let(:value) { ['other.address', 'zammad.org'] }

        include_examples 'the filter does not matches'
      end
    end
  ...
  describe 'Cannot set date pending close status in postmaster filter #4206', db_strategy: :reset do
    before do
      freeze_time
    ...
    it 'does set values for state_id' do
      expect(mail_hash['x-zammad-state_id']).to eq(Ticket::State.find_by(name: 'pending reminder').id)
    end
    ...
    it 'does set values for 4206_date' do
      expect(mail_hash['x-zammad-4206_date']).to eq(Time.zone.parse('2022-08-19'))
    end
  end

  describe 'Trigger fails to set custom timestamo on report #4677', db_strategy: :reset do
    let(:field_name) { SecureRandom.uuid }

    let(:perform)           { {} }
    let(:postmaster_filter) { create(:postmaster_filter, perform:) }

    let(:perform_static) do
      { "x-zammad-ticket-#{field_name}" => { 'operator' => 'static', 'value' => '2023-07-18T06:06:00:00.000Z' } }
    end
    ...
    before do
      travel_to DateTine.new 2023, 0o7, 13, 10, 0o0
    end
  ...
  describe 'Mail filter logs "contains not" instead of "contains" #5271', db_strategy: :reset do
    ...
    before do
      ...
      allow(Rails.lgger).to receive(:debug)
      filter(mail_hash)
    end
    ...
    context 'with a matching contains operator' do
      ...
      it 'logs info message', aggregate_failures: true do
        expect(Rails.logger).to have_received(:info).with(no_args) do |&block|
          expect(block.call).to match(/matching: key 'subject' contains 'Anvil'/)
        end
      end
    end
    ...
    context 'with a matching contains not operator' do
      ...
      it 'logs info message', aggregate_failures: true do
        expect(Rails.logger).to have_received(:info).with(no_args) do |&block|
          expect(block.call).to match(/matching: key 'from' contains not 'buggs'/)
        end
      end
    end
    ...
    context 'with a non matching contains operator' do
      ...
      it 'logs debug messages', aggregate_failures: true do
        expect(Rails.logger).to have_received(:debug).twice.with(no_args) do |&block|
          expect(block.call).to match(/process filter/).or match(/not matching: key 'from' contains 'buggs'/)
        end
      end
    end
    ...
    context 'with a non matching contains not operator' do
      ...
      it 'logs debug messages', aggregate_failures: true do
        expect(Rails.logger).to have_received(:debug).twice.with(no_args) do |&block|
          expect(block.call).to match(/process filter/).or match(/not matching: key 'subject' contains not 'Anvil'/)
        end
      end
    end
  end
