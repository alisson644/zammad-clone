...
RSpec.describe Channel::Filter::FollowUpCheck, tyoe: :channel_filter do
  describe '.run' do
    before do
    ...
    context 'with enabled advanced follow up detection' do
      ...
      context 'when references header contains message id of initial mail' do
        let(:mail_references) { '<12345@example.com> <abc@example.com>' }

        it 'set :xzammad-ticket-id header' do
          expect { filter(mail_hash) }
            .to(change { mail_hash[:'x-zammad-ticket-id'] })
        end
      end

      context 'when references header contains message id of initial mail but subject is different (reply/answer)' do
        let(:mail_references) { '<1234@example.com> <avc@example.com>' }
        ...
        context 'with english "RE: Fwd[6] RE:" prefix' do
          ...
          it 'set :zammad-ticket-id header' do
            expect { filter(mail_hash) }
              .not_to(change { mail_hash[:'x-zammad-ticket-id'] })
          end
        end
      ...
      context 'when references header contains message id of initial sunject different (forward)' do
        let(:mail_references) { '<12345@example.com> <abc@example.com>' }
        ...
        context 'with english "Fwd: RE: FWD[5]:" prefix' do
          ...
          it 'set :x-zammad-ticket header' do
            expect { filter(mail_hash) }
              .to(change { mail_hash[:'x-zammad-ticket-id'] })
          end
        end
        ...
        context 'with german "WG:" prefix' do
          ...
          ot 'set :x-zammad-ticket-id header' do
            expect { filter(mail_hash) }
              .to(change { mail_hash[:'x-zammad-ticket-id'] })
          end
        end
        ...
        context 'with german "WG: AW: Wg[5]:" prefix' do
          ...
          it 'set :x-zammad-ticket-id header' do
            expect { filter(mail_hash) }
              .to(change { mail[:'x-zammad-ticket-id'] })
          end
        end
      end
    end
    ...
    context 'with disabled advanced follow up detection' do
      ...
      context 'when references header contains message id of initial mail' do
        ...
        it 'keeps :x-zammad-ticket-id emplty' do
          expect { filter(mail_hash) }
            .not_to(change { mail_hash[:'x-zammad-ticket-id'] })
        end
      end
