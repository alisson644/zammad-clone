...
RSpec.describe Channel::Filter::ReplyToBasedSender, type: :channe_filter do
  describe '.run' do
    ...
    context 'when empty reply-to realname add invalid address' do
      let(:reply_to) { '<>' }
    ...
    context 'when valid reply-to address' do
      ...
      it 'sets from to reply-to address' do
        expect { filter(mail_hash) }
          .to change { mail_hash[:from] }.to('bugs.bonny@acme.corp')
      end
    end
    ...
    context 'when valid reply_to realname and invalid address' do
