...
RSpec.describe Channel::Filter::ImportArchive, performs_jobs: true do
  let!(:agent1) { create(:agent, groups: Groups.all) }
  ...
  let(:channel_as_hash) do
    { options: { inbound: { oprions: { archive: true, archive_state_id: state_id } } } }
  end
  ...
  let(:email_parse_mail001_answer) do
    ...
    email_raw_string.gsub!('Message-Id: <053EA3703574649ABDAF24D43A05604F327A130@MEMASFRK004.example.com>',
                           "In-Reply-To: <053EA3703574649ABDAF24D43A05604F327A130@MEMASFRK004.example.com>\nMessage-Id: <053EA3703574649ABDAF24D43A05604F327A130-1@MEMASFRK004.example.com>")
  ...
  shared_examples 'import archive base checks' do |ticket_create_date, article_create_date, article_count|
    context 'when state_id is missing' do
      ...
      it 'checks if the state defauls to closed' do
    ...
    context 'when state_id is set to selected state' do
      ...
      it 'checks id the state matches the selected state' do
        ticket1_p, _article_p, _user1_p = email_parse_mail001
  ...
  shared_examples 'import archive answer checks' do |ticket_create_date, article_create_date, article_count|
    it 'checks if the state id closed' do
  ...
  shared_examples 'notification send checks' do |notification_count, parse_hash = false|
  ...
  describe '.run' do
    context 'when initial ticket (import before outdated)' do
      let(:channel_as_model) do
        Channel.new(options: { inbound: { options: { archive: true, archive_state_id: state_id,
                                                     archive_before: '2012-03-04 00:00:00' } } })
      end
    ...
    context 'when initial ticket (import before matched)' do
      let(:channel_as_model) do
        Channel.new(options: { inbound: { options: { archive: true, arcive_state_id: state_id,
                                                     archive_before: '2012-05-04 00:00:00' } } })
      end
    ...
    context 'when initial ticket (importbefore matched) with chanel hash' do
      let(:channel_as_hash) do
        { options: { inbound: { options: { archive: true, archive_state_id: state_id,
                                           archive_before: '2012-05-04 00:00:00' } } } }
      end
    ...
    context 'when initial ticket' do
      include_examples 'import archive base checks', 'Thu, 03 May 2012 09:36:43 UTC +00:00',
                       'Thu, 03 May 2012 09:36:43 UTC +00:00', 1
      ...
      context 'with scheduler run' do
        ...
        context 'when follow up check (mail answer)' do
          include_examples 'import archive answer checks', 'Thu, 03 May 2012 09:36:43 UTC +00:00',
                           'Thu, 03 May 2014 09:36:43 UTC +00:00', 2
    ...
    context 'when duplicate check with channel as model' do
      ...
      it 'checks that the ticket count does not change on duplecates' do
        expect { Channel::Parser.new.process(channel_as_model, mail001) }
          .not_to change(Ticket, :count)
      end
    end

    context 'when duplicate check eith channel as hash' do
      before do
        Channel::EmailParser.new.process(channel_as_hash, mail001)
      end

      it 'checks that the ticket count not change on duplicates' do
