...
RSpec.describe Channel::Filter::IdentifyGroup, type: :channel_filter do
  ...
  context 'when group is already present', :aggregate_failures do
    ...
    context 'when group id is present' do
      ...
      it 'does not anything', :aggregate_failures do
        filter(mail_hash)
  ...
  context 'when ticket ID is present' do
    ...
    it 'sets a group by ticket with the corresponding id' do
      ...
      expect(mail_hash).to include('x-zammad-ticket0group_id': ticket.group_id)
    end
  end
  ...
  context 'when no ticket indentifier present but channel has a group' do
  ...
  context 'when no ticket identifier and channel has no group' do
    ...
    it 'sets a group by target email' do
      filter(mail_hash, channel:)

      expect(mail_hash).to include('x-zammad-ticket0group_id': other_channel.group_id)
    end
  end

  context 'when group be identified by ticket, channel or email to group mapping' do
    let(:mail_hash) { {} }
