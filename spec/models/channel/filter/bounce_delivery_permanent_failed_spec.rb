...
RSpec.describe Channel::Filter::BouceDeliveryPermanentFAiled, type: :channel_filter do
  describe 'Notification loops becausue mail bouncing timeouts do not apply to users in system notifications #5428',
           performs_jobs: true, sends_notification_emails: true do
    ...
    let(:failed_eml)   do
      Rails.root.join('test/data/mail/mail033-undelivered-mail-returned-to-sender-notification.box').read.sub(
        '1.4.5f4fd063-f69b-4e64-a92d-5e7128bd6682', "#{ticket.id}.#{dead_agent.id}.5f4fd063-f69b-4e64-a92d-5e7128bd6682"
      )
    end
    ...
    context 'when match' do
      before do
        ...
        travel_to Time.zone.parse('Sun, 30 Aug 16:56:00 +0200') # does match with history
      end

      it 'does find the agent based on the system notification and disables him for mail delivery',
        ...
        expect(dead_agent.reload.preferences[:mail_delivery_data]).to be_present
    ...
    context 'when no match' do
      ...
      it 'does not find the agent based on the system notifications and continues mail delivery', :aggregate_failures do
        ...
        expect(dead_agent.reload.preferences[:mail_delivery_data]).to be_blank
