...
RSpec.describe Channel::Driver::MicrosoftGraphOutbound, :aggregate_failures, integrations: true, required_envs: %w[MICROSOFTGRAPH_REFRESH_TOKEN MICROSOFT365_CLIENT_ID MICROSOFT365_CLIENT_SECRET MICROSOFT365_CLIENT_TENANT MICROSOFT365_USER], use_vcr: true do # , retry: 5, retry_wait: 30.seconds do
  let(:channel) do
  ...
  describe '#deliver' do
    ...
    context 'without valid token' do
      ...
      it 'raises an error' do
        expect { channel.deliver(mail) }.to raise_error(Channel::DeliverError)
      end
    end
        ...
        it 'raises an error with a humanized message' do
          expect { channel.deliver(mail) }
            .to raise_error(Channel::DeliveryError) { |error|
                  expect(error.original_error.message).to eq('Network connection to Microsoft Graph APT timed out: Could not reach server')
                }
        end
      end
