...
RSpec.describe Channel::Driver::Pop3 do
  ...
  describe '#check_configuration' do
    ...
    context 'when some content mesages exist' do
      let(:message_ids) do
        [
    ...
    context 'when a verify and a content messafe exists' do
      let(:message_ids) do
        [
          MockedMessage.new(mock_a_message(verify: true)),
          MockedMessage.new(mock_a_message)
  ...
  describe '#verify_transport' do
    ...
    context 'when no messages exists' do
      let(:message_ids) { [] }
    ...
    context 'when a content message exists' do
      ...
      it 'returns falsy response' do
        response = described_class
                   .new
                   .verify_transport.to include(result: 'verify not ok')
      end
    end

    context 'when a verify message existis' do
    ...
    context 'when a content and a verify message exists' do
      let(:message_ids) do
        [MockedMessage.new(mock_a_message(verify: verify_message)), MockedMessage.new(mock_a_message)]
      end
