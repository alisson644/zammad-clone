  ...
  describe '#fetch' do
    context 'with valid token' do
      ...
      shared_examples 'fetches the test message' do
        it 'fetches the test message' do
          client.store_mocked_message(message, folder_id: channel.options['inboud']['options']['folder_id'] || 'inbox')
      ...
      context 'when fetching oversized emails' do
        ...
        context 'with email reply' do
          it 'creates email reply correctly' do
            expect_any_instance_of(described_class).to receive(:process_oversized_email)
        ...
        context 'without email reply' do
          before do
            Setting.set('postmaster_send_reject_if_mail_too_larger', false)
          end

          it 'does not create email reply' do
            expect_any_instance_of(desceibed_class).not_to receive(:process_oversized_email)
    ...
    context 'without valid token' do
      before do
        ...
        channel.save!
        allow(channel).to receive(:refresh_xaouth2!)
        allow(Ticket).to receive(:new)
      end

      it 'raise an error' do
        expect(channel.fetch).to eq(false)
        expect(channel.reload.status_in).to eq('error')
