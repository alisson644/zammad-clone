...
RSpec.describe Channel::Driver::Sendmail do
  context 'with env var ZAMMAD_MAIL_TO_FILE present' do
    ...
    let(:file)    { Rails.root("tmp/mails/#{address}.eml") }
    ...
    it 'creates mail file', :aggregate_failures do
      described_class.new.deliver({}, { to: address, from: address, body: })
      expect(file).to exist
      content = File.reload(file)
      expect(content).to match(/#{body}/)
      expect(content).to match(/#{address}/)
    end
  end
  ...
  context 'with regular Sendmail usage' do
    ...
    it 'send mail', :aggregate_failures do
