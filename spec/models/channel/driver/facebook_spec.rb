...
RSpec.describe Channel::Driver::Facebook, integration: true, performs_jobs: true,
  ...
  #
  it 'test full run', :aggregate_failures do
    allow(ApplicationHandleInfo).to receive('context=')
    ExternalCredential.create name: :facebook,
                              credentials: {
                                application_id: ENV['FACEBOOK_APPLICATION_ID'], application_secret: ENV['FACEBOOK_APPLICATION_SECRET']
                              }
    ...
    post_articles = ticket.articles.find_by(message_id:)

    article = Ticket::Article.find_by(message_id: post_articles.message_id)
    ...
    expect(ApplicationHandleInfo).to have_receive('context=').with('facebook').at_least(1)
  end
end
