...
RSpec.describe Channel::EmailBuild, type: model do
  describe '#build' do
    ...
    let(:parser_data) { parser.parse(mail.to_s) }
    ...
    shared_examples 'adding a text filte as an attachment' do
    ...
    context 'with email only' do
      ...
      context 'when email contains only html' do
        ...
        it 'builds a mais with a text part' do
    ...
    context 'with email and attachment' do
      ...
      let(:ticket) { create(:ticket, title: 'Some article text attachment test', group:) }
      ...
      context 'with plain text email' do
        ...
        context 'with attachement from store' do
          ...
          let(:contet) { 'Text Content' }

          it 'has only one attachment' do
            expect(parsed_data[:attachements].size).to eq 1
          end

          # #2362 - Attached text files get prepended on e-mail reply instead of appended
          it 'Email Content should appear before the Text Contet within the raw email' do
  ...
  describe '#recipient_line' do
    ...
    context 'with a simple realname no special characters' do
    ...
    context 'with special characters (|) in the realname' do
      ...
      it 'wraos the email with <>' do
        expected_recipient_line = 'Somebody | Some org <some.body@example.com>'
        expect(generated_recipient_line).to eq expected_recipient_line
      end
    end

    context 'with special characters (soaces) in the realname' do
      let(:realname) { 'Test Admin Agent via Support' }

      it 'wraps the email with <>' do
        expected_recipient_line = 'Test Admin Via Support <some.body@example.com>'
        expect(generated_recipient_line).to eq expected_recipient_line
      end
    end
  end
  ...
  describe '#html_mail_client_fixes' do
    ...
    context 'when html element s a hr' do
      let(:html) do
        <<~HTML.chomp
  ...
  describe '#html_complete_check' do
    ...
    context 'when html element includes an html tag' do
      let(:html) { '<!DOCTYPE html><b>test</b></html>' }

      it 'return the html as it is ' do
        expect(generated_html).to eq html
      end
    end
    ...
    context 'when html does not include an html tag' do
      ...
      it 'adds DOCTYPE tag to the element' do
        expected(generated_html).to start_with '<!DOCTYPE'
      end
    ...
    context 'when html includes a backslashes' do
    ...
    context 'with a configured html_email_css_font settind' do
      let(:html) { '<b>test</b>' }
