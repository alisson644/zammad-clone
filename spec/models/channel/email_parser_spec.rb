  ...
  describe '#parse' do
    ...
    context 'when checking a bunch of stored emails for correct parsing behaviour' do
      Rails.root.glob('test/data/mail/mail*.box').each do |stored_email|
        next if ENV['MAIL_PARSER_TEST'] && stored_email.to_s.exclude?("#{ENV['MAIL_PARSER_TEST']}.box")
  ...
  describe '#process' do
    ...
    describe 'auto-creating new users' do
      ...
      context 'with existing system email address' do
        ...
        let!(:group)         { create(:group, name: 'baz headquarter', email_address:) }
        let!(:channel) do
          channel = create(:email_channel, group:)
          email_address.update(channel:)
          channel
        end
    ...
    describe 'association emails to existind tickets' do
      ...
      describe 'based on where a ticket reference appears in the message' do
        ...
        shared_context 'ticket reference in In-Reply-To header' do
          ...
          let!(:article) do
            create(:ticket_article, ticket:, message_id: '<20150830145601.30.608882@edenhofer.zammad.com>')
          end
        end
        ...
        context 'when configured to search body' do
          ...
          context 'when body contains ticket reference' do
            ...
            context 'as part of a larger word' do
              let(:ticket_ref) do
                "Foo#{Setting.get('ticket_hook')}#{Setting.get('ticket_hook_divider')}#{ticket.number}bar"
              end
            ...
            context 'in html attributes' do
              let(:ticket_ref) do
                %(<table bgcolor="#{Setting.get('ticket_hook')}#{Setting.get('ticket_hook_divider')}#{ticket.number}"> </table>)
              end
