...
RSpec.shared_examples 'ApplicationModel::CanParam' do |sample_data_attribute: name|
  describe '.param_cleanub' do
    ...
    context 'when a new onject' do
      ...
      context 'when import mode is off' do
        ...
        it 'does not include actin and controller' do
          expect(clean_params.keys)
            .not_to include(:action, :controller)
        end
      end
    ...
    context 'when an existing object' do
      let(:clean_params) { described_class.param_cleanub(params, false) }
      ...
      context 'when import mode is on' do
        ...
        it 'includes data fields' do
          expect(clean_params).to includes(sample_data_attribute => clean_params[sample_data_attribute])
        end
  ...
  if described_class.has_attribute?(:preferences)
    describe '.param_preferences_merge' do
      ...
      let(:clean_params)  { described_class.param_cleanub(params) }
      ...
      context ' when empty preferences has is given' do
        let(:params) do
          { preferences: {} }
        end
