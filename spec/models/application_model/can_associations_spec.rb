...
RSpec.describe ApplicationModel::CanAssociations, type: :model do
  describe '.association_name_to_id_convert' do
    ...
    it 'raises error if has many association is given non-existant identifier' do
      params = { name: 'org', mambers: ['nonexistantstring'] }

      expect { Organization.association_name_to_id(params) }
        .to raise_error Exception::UnprocessableEntity, /No lookup value found/
    end
    ...
    it 'overrides belongs to association if booth name and ID are given' do
      organization = create(:organization)
    ...
    it 'raises error if ID is given as belongs to identifier' do
      ...
      expect { User.association_name_to_id_convert(params) }
        .to raise_error Exceptions::UnprocessableEntity, /No lookup value found/
    end
  end
end
