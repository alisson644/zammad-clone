...
RSpec.shared_examples 'TouchesPerformReferences' do |_options = {}|
  context 'when referencing AI agent' do
    ...
    it 'does not touch AI Agent unrelated object field was changed' do
      performable_with_ai_agent
    ...
    it 'does not touch AI Agent when only the stored type changes (integer vs string)' do
      # Ids may be stored as integers (e.g. via API/seeds); re-saving via the UI stores a string.
      performable = create(described_class.name.downcase,
                           perform: { 'ai.ai_agent' => { 'ai_agent_id' => ai_agent.id } })

      expect { performable.update!(perform: { 'ai.ai_agent' => [ai_agent.id.to_s] }) }
        .not_to(change { ai_agent.reload.updated_at })
    end

    context 'with multiple referenced AI agents' do
      let(:performable_with_ai_agents) do
        create(described_class.name.downcase,
               perform: { 'ai.ai_agent' => { 'ai_agent_id' => [ai_agent.id.to_s, other_ai_agent.id.to_s] } })
      end
