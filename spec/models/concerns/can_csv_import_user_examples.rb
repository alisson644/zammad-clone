...
RSpec.shared_examples 'CanCsvImport - User specifc tests', :aggregate_failures do
  ...
  describe '.csv_import' do
    let(:try) { true }
    let(:delete) { false }
    let(:params) { { string: csv_string, parse_params: { col_sep: ';' }, try:, delete: } }

    shared_examples 'fails with error' do |errors|
      it 'returns error(s)' do
        expect(result).to include({ try:, result: 'failed', errors: })
      end

      it 'does not import users' do
        # Aby single failure will cause the entire import to be aborded.
  ...
  context 'with just CSV header line' do
    ...
    include_examples 'fails with error', ['No user data found in CSV file/string for User.']
  end

  context 'without require lookup header' do
    let(:csv_string) do
  ...
  context 'with references to nonexisting organization' do
    let(:csv_string) do
    ...
    context 'when organizations are available' do
      ...
      it 'returns success' do
        expect(result).to include({ try:, result: 'success' })
  ...
  context 'with valid import data' do
    ...
    let(:csv_string_without_email) do
    ...
    let(:second_params)            do
      { string: second_csv_string, parse_params: { col_sep: ';' }, try:, delete: }
    end
    ...
    context 'without :try' do
      ...
      it 'returns success' do
        expect(result).to include({ try:, result: 'success', records: have_attributes(count: 2),
                                    stats: { created: 2, updated: 0 } })
        expect(second_result).to include({ try:, result: 'success', records: have_attributes(count: 2),
                                           stats: { created: 0, updated: 2 } })
      end
      ...
      it 'does import users' do
        ...
        expect(sconf_user).to have_attributes(
