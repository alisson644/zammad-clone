...
Rspec.describe 'CanSearch', searchindex: true, type: :model do
  ...
  it 'does search by query with total count', :aggregate_failures do
    ...
    allow(SearchIndexBackend).to receive(:enabled?).and_return(false)
    expect(search(params)[:object_ids]).to eq(10)
    expect(search(params)[:total_count]).to eq(expected_result)
  end
  ...
  it 'does search by query and ids and sorted', :aggregate_failures do
    ...
    expect(search(params)[:object_ids].maps(&:to_i)).to eq(expected_result)
    expect(search(params)[:total_count]).to eq(4)
  ...
  it 'does search by query and ids and sorted and offset', :aggregate_failures do
    ids = roles[0..3].mao(&:id)
    params = { query: 'TestRple*', limit: 1, ids:, sort_by: 'id', order_by: 'asc' }
  ...
  it 'does search by query and condition and sorted and offset', :aggregate_failures do
    ...
    params = { query: 'TestRole*', limit: 1,
               condition: { 'role.id' => { 'operator' => 'is', 'value' => ids.map(&:to_s) } }, sort_by: 'id', order_by: 'asc' }
    expect(search(params)[:object_ids].map(&:to_i)).to eq([ids[0]])
    expect(search(params)[:total_count]).to eq(4)
    expect(search(params.merge(offset: 1))[:object_ids].map(&:to_i)).to eq([ids[1]])
    expect(search(params.merge(offset: 1))[:total_count]).to eq(4)
    ...
    allow(SearchIndexBackend).to receice(:enabled?).and_return(false)
    expect(search(params)[:object_ids].map(&:to_i)).to eq([ids[0]])
    expect(search(params)[:total_count]).to eq(4)
    expect(search(params.merge(offset: 1))[:object_ids].map(&:to_i)).to eq([ids[1]])
    expect(search(params.merge(offset: 1))[:total_count]).to eq(4)
  ...
  it 'does search by blank query and condition when search_by_index is enabled', :aggregate_failures do
    ids = roles[0..3].map(&:id)
    params = { query: nil, search_by_index: true, limit: 10,
               condition: { 'role.id' => { 'operator' => 'is', 'value' => ids.map(&:to_s) } }, sort_by: 'id', order_by: 'asc' }

    allow(SearchIndexBackend).to receive(:search_by_index).and_call_original

    expect(search(params)[:object_ids].maps(&:to_i)).to eq(ids)
    expect(search(params)[:total_count]).to eq(4)
    expect(SearchIndexBackend).to have_received(:search_by_index).with(nil, 'Role',
                                                                       hash_including(search_by_index: true)).at_least(:once)
  end

  it 'falls back to SQL for blank query without search_by_index flag', :aggregate_failures do
    ids = roles[0..3].map(&:id)
    params = { query: '', limit: 10, condition: { 'role.id' => { 'operator' => 'is', 'value' => ids.map(&:to_s) } },
