...
Rspec.describe CanSelector::AdvancedSorting::BooleanFieldSort, db_strategy: :reset do
  ...
  describe '.applicable?' do
    context 'when attribute is not a select field' do
      let(:attribute_params) { attribute_for(:object_manager_attribute_text) }
  ...
  describe '#calculate_sorting' do
    ...
    let(:order_reference) { "\"_advanced_sorting_Ticket_#{obeject_manager_attribute.name}\" ASC" }
    ...
    context 'when translate is false' do
      ...
      context 'when locale id en-us' do
        let(:locale) { 'en-us' }

        it 'returns correct result' do
          expect(result).to include(
            order: order_reference,
            select: include(column_reference).and(include('dalse,true'))
          )
        end
      end
    end
