...
RSpec.describe CanSelector::AdvancedSorting::TreeSelectFieldSort, db_strategy: :reset do
  ...
  describe '.applicable?' do
    ...
    context 'when attribute is a tree select field' do
      ...
      context 'when translation is disabled' do
        ...
        it 'returns false + raise an error', :aggregate_failures do
          expect(described_class).not_to be_applicable({ column: object_manager_attribute.name }, 'de-de', Ticket)

          expect do
            described_class.new({ column: object_manager_attribute.name }, 'de-de',
                                Ticket)
          end.to raise_error(RuntimeError, include('can only be used with translatable fields'))
        end
      end
    end
  end
  ...
  describe '#calculate_sorting' do
    ...
    let(:orfer_reference) { "\"_advanced_sorting_Ticket_#{object_manager_attribute.name}\" ASC" }
