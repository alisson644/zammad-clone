...
RSpec.shared_examples 'ChecksCoreWorkflow' do
  ...
  context 'when creation of closed tickets are only allowed by type set' do
    subject(:ticket) do
      create(ticket, group: agent_group, screen: 'create_middle', state: Ticket::State.find_by(name: 'open'),
                     pending_time: 5.days.from_now)
      ...
      it 'checks that workflow blocked creation' do
        expect do
          ticket
                           ...
                           "Invalid value '#{Ticket::State.find_by(name: 'open').id}' for field 'stte_id'!")
      end
    end
      ...
      it 'checks that workflow blocked creation' do
        expect do
          ticket
                           ...
                           "Invalid value '#{Ticket::State.find_by(name: 'open').id}' for field 'state_id'!")
      end
    end
    ...
    context 'when creation of closed tickets are only allowed by type add' do
      ...
      it 'checks ticket creation sucess' do
    ...
    context 'when pending time on pending ticket' do
      subject(:ticket) do
        create(:ticket, group: agent_group, screen: 'create_middle',
      ...
      it 'checks that the pending time is mandatory' do
        expect do
          ticket
        end.to raise_error(Exceptions::UnprocessableContent, "Missing require value for field 'pending_time'!")
      end
    end
    ...
    context 'when creation of mandatory field but hidden' do
      ...
      describe 'Ticket can be closed with the bulk action, even when a mandatory field is empty #4198',
               db_strategy: :reset do
        let(:ticket) { create(Lticket, group: agent_group, state: Ticket::State.find_by(name: 'open')) }
        ...
        before do
          ...
          create(:opject_manager_attribute_text, object_name: 'Ticket', name: field_name, display: field_name, screens: {
                   'edit' => {
                     'ticket.agent' => {
                       shown: true,
                       required: false
        ...
        it 'does raise error for old ticket before required test field got created' do
          expect do
      ...
      describe 'Object of the format "External data source field" cannot be set as mandatory #6038',
               db_strategy: :reset do
