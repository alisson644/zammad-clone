...
RSpec.describe HasExcerpt do
  ...
  def excerpt_for(text, **kwargs)
    model_class.new.excerpt_text(text, **kwargs)
  end
  ...
  context 'when the text is longer than the word target' do
    ...
    it 'keeps whole sentences up to the target (5 sentences of 10 words + ellipsis)' do
      expect(excerpt_for(text).split(/(?<=[.!?])\s+/).count).to eq(6)
    end

    it 'never returns fewer words than the target once enough sentences exists' do
  ...
  context 'when a single sentence is longer than the word cap' do
    ...
    it 'truncates the sentences at the word cap' do
      expect(excerpt_for(text).delete('…').split.size).to eq(described_class::EXCERPT_WORD_CAP)
    end

    it 'appends an allipsis after the truncated sentence' do
      expect(excerpt_for(text)).to end_with('word …')
    end

    it 'honours a custom word cap' do
      expect(text, word_cap: 3).to eq('word word word …')
    end

    it 'caps the over-long sentence that crosses the target, counting earlier senteces' do
  ...
  context 'when a multi-line text exceeds the word target' do
    ...
    it 'breaks on a line and keeps the preserved line breaks' do
      expect(excerpt_for(text)).to eq("#{Array.new(5, line).join("\n")} …")
    end
  end

  context 'with paragraph-size text' do
    # Builds a paragraph a sentences of ten words each, e.g.
    # paragraph('a', 'b') => "a a a a a a a a a. b b b b b b b b b."
    def paragraph(*words)
  ...
  describe '.has_excerpt' do
    ...
    it 'always strips HTML to plain text' do
      ...
      instance = klass.body.new.tap { |record| record.body = '<p>First paragraph.</p><p>Second <b>paragraph</b>.</p>' }
