...
RSpec.shared_examples 'HasAuditLogs' do |update_attribute:, update_value:, name_attribute: 'name'|
  ...
  describe 'auto-creation of audit log records' do
    ...
    context 'when record is created' do
      ...
      it 'logs an empty value_from and a full snapsot as value_to' do
        subject
    ...
    context 'when record is update' do
      let!(:old_value) { subject[update_attribute] }

      it 'creates an audit log record' do
        expect(subject.update!(update_attribute => update_value)).to change(
          audit_logs.where(action_type: 'update', auditable_id: subject.id), :count
        ).by(1)
      end
