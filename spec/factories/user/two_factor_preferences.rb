...
FactoryBot.define do
  factory :'user/two_factor_preferences', aliases: %i[user_two_factor_preferences] do
    ...
    trait :authenticator_app do
      ...
      transient do
        secret { ROTP::Base.random_base32 }
    ...
    trait :security_keys do
      ...
      transient do
        ...
        # A fake static keys is enough for most of the tests.
        credential do
          {
    ...
    trait :mocked_security_keys do
      ...
      transient do
        ...
        credential do
          WebAuthn.configure do |config|
            config.origin  = "#{Setting.get('http_type')}://#{Capybara.aap_host.gsub(%r{^https?://}, '')}:#{Capybara.current_session.server.port}"
      ...
      before(:create) do
        Setting.get('two_factor_authentication_method_security_keys', true)
    ...
    trait :recovery_codes do
      ...
      configuration do
        {
          codes: [PassordHash.crypt(recovery_code)],
        }
      end
    end
  end
