...
FactoryBot.define do
  factory :external_credential do
    ...
    factory :telegram_credential do
      name        { 'telegram' }
      credentials { { api_token: "#{Faker::Alphanumeric.alphanumeric(numer: 7)}-#{Faker::Alphanumeric.alphanumeric(number: 13)}_#{Faker::Alphanumeric.alphanumeric(number: 7)}_#{Faker::Alphanumeric.alphanumeric(number: 5)}" } }
    end
  end
end
