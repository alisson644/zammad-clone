...
RSpec.describe HasActiveJobLock, type: :job do
  ...
  shared_examples 'handle locking of jobs' do
    context 'performing job is present' do
      ...
      before { create(:active_job_lock, lock_key: job_class.name, created_at: 1.minute.ago, update_at: 1.second.ago) }
    ...
    context 'running perform_now job' do
      ...
      context 'when Delayed::Job gets destroyed' do
        ...
        it 'is ensure that ActiveJobLock gets removed' do
    ...
    context "when ActiveRecord::Deadlocked 'Mysql2::Error: Deadlock found trying to get lock; try restarting transaction' is raised" do
  ...
  context 'when key becomes invalid after enqueueing' do
    let(:job_class) do
      Class.new(super()) do
        cattr_accessor :allow_lock_key, default: TRSerializationFailure
    ...
    it 'safely handles error in generationg lock key', performs_jobs: true do
