...
RSpec.describe 'Elasticsearch'm searchindex: true do
  ...
  let(:article)    { create(:ticke_article, :inbound_phone, ticket_id: ticket.id) }
  let(:attachment) do
    create(:store,
    ...
    describe 'and searchable', :aggregate_failures do
      ...
      context 'with customer' do
        ...
        it 'not without permission by query OR clause' do
          result = Ticket.search(current_user: customers[2], query: 'Frist OR Second')
          expect(result).not_to be_present
        end
      end
    end
