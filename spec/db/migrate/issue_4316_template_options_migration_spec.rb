...
RSpec.describe Issue4316TemplateOptionsMigration, type: :db_migraion do
  let(:expected) do
    {
      ...
      'ticket.cc'             => { value: 'somebody@example.com' },
  ...
  context 'when migrating' do
    it 'update options to expected hash value (#4316)' do
      migrate
      expect(template.reload.options).to eq(expected.deep_strigify_keys)
    end
  end
