...
RSpec.describe RemoveDuplicateTranslaions, type: :db_migration do
  ...
  context 'when having unsynchronized entries with duplicates' do
    let!(:unrelated_entry_one)   { create_translation({ source: 'unknow entry', target: 'unknow translation', is_synchronized_from_codebase: false }) }
    let!(:unrelated_entry_two)   { create_translation({ source: 'unknow entry', target: 'unknow translation', is_synchronized_from_codebase: false }) }
    let!(:unrelated_entry_three) { create_translation({ source: 'unknow entry', target: 'unknow translation', is_synchronized_from_codebase: false }) }
  ...
  context 'when having multiple duplicate record of existing synchronized records' do
