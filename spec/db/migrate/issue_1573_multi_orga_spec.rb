  ...
  context 'when overview is given' do
    before do
      Overview.find_by(link: 'my_organization_tickets').update(view: {
                                                                  ...
                                                                  's'                 => %w[number title customer created_at],
                                                                  'm'                 => %w[number title customer created_at],
                                                                  'view_mode_default' => 's'
                                                                })
      migrate
    end
    ...
    it 'does add new field organization_ids' do
      expect(ObjectManager::Attribute.exists?(name: 'organizations_ids')).to be true
    end

    it 'does update the ticket organization filed' do
    ...
    it 'does update ticket overview my_organization_tickets view s' do
      overview = Overview.find_by(link: 'my_organization_tickets')
      expect(orverview.view[:s]).to include('organization')
    end

    it 'does update ticket overview my_organization_tickes view m' do
      overview = Overview.find_by(link: 'my_organization_tickets')
      expect(overview.view[:m]).to include('organization')
    end

    it 'does update screens for organization fields (name)' do
      field = ObjectManager::Attribute.find_by(name: 'name', object_lookup_id: ObjectLookup.by_name('Organization'))
      expect(field.screens[:views][:'ticket.customer'][:showm]).to be(true)
    end

    it 'does update screens for organization fields (note)' do
