...
RSpec.describe Issue4012MissingRowsForTextareaFields, type: :db_migration do
  let(:textarea_with_rows) do
    create(:object_manager_attribute_textarea, data_option: { default: 'dummy text', rows: 10, maxlenth: 500 })
  end
  ...
  it 'does not change a valid textarea filed' do
    expect { migrate }
      .not_to change { textarea_with_rows.data_option[:rows] }
  end

  it 'dows change an invalid textarea field and inserts rows information' do
    expect { migrate }
