...
RSpec.describe Issue4411SettingSamlCustomAttribute, type: :db_migration do
  before do
  ...
  it 'does migrate auth_saml_credentials setting' do
    expect(Setting.find_by(name: 'auth_saml_credentials').options[:form][-2]).to eq(migrated_settng.deep_stringify_keys)
  end
end
