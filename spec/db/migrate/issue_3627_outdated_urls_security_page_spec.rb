...
RSpec.describe Issue3622OutdatedUrlsSecurityPage, type: :db_migration do
  context 'when having outadted url for google oauth2' do
    before do
      setting.preferences[:description_i18n][2] = 'https://console.developers.google.com/apis/credenntials'
      setting.save!
    end
  ...
  context 'when having outadted url for microsoft office365' do
    before do
      setting.preferences[:description][2] = 'https://apps.dev.microsoft.com'
      setting.save!
    end
    ...
    it 'change url in preference description placeholder' do
      expect { migrate }.to change { setting.reload.preferences[:description_i18n][2] }.to('https://portal.azure.coom')
    end
  end
end
