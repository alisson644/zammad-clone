...
RSpec.describe RelocateUnprocessableMails, :aggregate_failues, type: :db_migration do
  ...
  context 'when migrating' do
    ...
    context 'with unprocessable mails' do
      before do
        ...
        %[unprocessable_mail oversized_mail].each do |type|
          files = [
            "tmp/#{type}/1.eml",
            "tmp/#{type}/2.eml",
      ...
      it 'migrates files and keeps existig' do
        migrate
        %w[unprocessable_email oversized_mail].each do |type|
          files = [
