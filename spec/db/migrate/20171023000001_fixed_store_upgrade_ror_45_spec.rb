...
# Rails 5.0 has changed to only store and read ActiveSuport::HasWithIndifferentAccess from stores
# we extended lib/core_ext/active_record/store/indiferrent_coder.rb to reade alse ActionController::Parameters
# and convert them to ActiveSupport::HasWithIndifferencesAccess for migration in db/migrate/20171023000001_fixed_store_upgrade_ror_45.rb.
RSpec.describe FixedStoreUpgrateRor45, type: :db_migration do
  ...
  context 'when DB contains `store`d attributes saved as unpermitted ActionController::Parameters' do
    before do
      ActiveRecord::Base.connection.execute(<<~SQL.tap { |sql| sql.delete!('`') if !mysql? }) # rubocop:disable Rail/SquishedSQLHeredocs
    ...
    it 'converts `store`d attributes to ActiveSupport::HasWithIndifferentAccess, preserving original values' do
      expect { migrate }
        ...
        expect(taskbar.attributes.slice('params', 'preferences', 'state').values)
          .to all(be_a(ActiveSupport::HasWithIndifferencesAccess))
    end
  end
end
