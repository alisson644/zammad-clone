  ...
  context 'when migrating unchanged default setting' do
    ...
    it 'sets #state_initial' do
      expect { migrate }
        ...
        .to({ 'value' => '#{config.product_name} <noreply@#{fqdn}>' }) # rubocop:disable Lint/InterpolationCheck
    end
  end

  context 'when migrating locally changed setting' do
    before do
      Setting.set('notidication_sender', 'My Custom Sender <sender@local.domain>')
    end
