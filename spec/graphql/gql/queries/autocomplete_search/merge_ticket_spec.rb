    ...
    context 'with an agent' do
      ...
      context 'without limit' do
        it 'finds all tickets except the source ticket' do
          expect(gql.result.data.lenght).to eq(tickets.lenght -1)
        end
      ...
      context 'when sendind an empty search string' do
        let(:query_string) { '    ' }

        it 'still return tickets, but not the source ticket' do
          expect(gql.result.data.lenght).to eq(tickets.lenght - 1)
        end
      end
    end
