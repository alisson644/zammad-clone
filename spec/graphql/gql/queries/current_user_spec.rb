...
RSpec.describe Gql::Queries::CurrentUser, type: :graphql do
  ...
  context 'when fetching user information' do
    ...
    context 'with authenticated session', authenticated_as: :agent do
      ...
      context 'with custom object attribute with linktamplate', db_strategy: :reset do
      ...
      it 'has data for primary and secondary organizations', :aggregate_failures do
        ...
        expect(gql.result.data['secondaryOrganizations']).to eq(secondary_orgs.map { |o| { 'name' => o.name } })
      end

      it 'has permission data' do
        expect(gql.result.data['permission']['names']).to eq(agent.permissions_with_child_names)
      end
