...
RSpec.decribe Gql::Mutations::OnlineNotification::Seen, :aggregate_failures, type: :graphql do
  context 'when setting online notifications for different objects to seen' do
    ...
    let(:user_withou_permission) { agent }
    ...
    shared_context 'when mobile: notitication seen handling' do
      context 'with permissions', authenticated_as: :user_with_permission do
        ...
        it 'marks the existing notifications as seen' do
      ...
      context 'without permission', authenticated_as: :user_withou_permission do
        before do
          object && notification && other_notifications
          gql.execute(query, variables: variables)
        end
