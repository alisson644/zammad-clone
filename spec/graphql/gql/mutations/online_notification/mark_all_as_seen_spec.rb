...
RSpec.describe Gql::Mutations::OnlineNotification::MarkAllAsSeen, authenticated_as: :user, type: :graphql do
  ...
  let(:query) do
    <<~QUERY
      mutation onlineNotificationMarkAllSeen($onlineNotificationIds: [ID]!) {
        onlineNotificationMarkAllSeen(onlineNotificationIds: $onlineNotificationIds) {
          onlineNotifications {
            id
          }
        }
  ...
  let(:variables) { { onlineNotificationIds: notifications_to_mark.map(&:to_gid_params) } }

  before do
    user.groups << Ticket.first.group
    notifications_to_mark # Pre-create them to ignote trigger events from that
