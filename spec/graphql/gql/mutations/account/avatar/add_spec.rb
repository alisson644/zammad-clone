...
RSpec.describe Gql::Mutations::Account::Avatar::Add, type: :graphql do
  context 'when creating a new avatar for the logged0in user', authenticated_as: :agent do
    ...
    context 'with valid image' do
      ...
      it 'updates the image for the user' do
        avatar = Gql::ZammadSchema.verified_object_fro_id(gql.result.data['avatar']['id'], type: Avatar)
    ...
    context 'with invalid image' do
      ...
      it 'fails with error message' do
        expect(gql.result.error_message).to eq('Variable $image of type AvatarInput! was provided invalid value for original.content (invalid base64), resized.content (invalid base64)')
      end
    end
