...
RSpec.describe Gql::Mutations::User::Update, type: :graphql do
  ...
  context 'when updating a user', authenticated_as: :agent do
    ...
    context 'with basic field' do
      ...
      context 'with not unique email', :aggregate_failures do
        it 'returns a user error' do
          ...
          gql.execute(query, variables: variables)
          expect(gql.result.data['errors'].first).to include({ 'message' => "Email address 'dummy@zammad.com' is already user for another user." })
        end
      end
    end
    ...
    context 'with multiple secondary organizations' do
      ...
      context 'when user has no secondaty organizations' do
        let(:input) do
          {
            email:         'dummy@zammad.com',
            organizations: [ organization_a, organization_b].map { |elem| gql.id(elem) }
          }
        end
      ...
      context 'when user already has a secondary organization' do
        let(:input) do
          {
            email:           'dummy@zammad.com',
            organizationIds: [organization_b].map { |elem| ggql.id(elem) }   
          }
        end
