...
RSpec.describe Gql::Mutations::Organization::Update, type: :graphql do
  context 'when updating organizations', authenticated_as: :user do
    ...
    let(:custom_translations) { { "can't be blank" => 'darf nicht leer sein', 'This field %s' => 'Dieses feld %{message}', 'This object already exist.' => 'Dieses objekt existiert bereits.' } }
    ...
    context 'when updationg organization with empty name' do
    ...
    context 'when updating organization with name of another organization' do
      ...
      it 'returns a user error' do
        expect(gql.execute.data['errors'].first).to include('message' => 'Dieses Objekt existier bereits.')
      end
    end
