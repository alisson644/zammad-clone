  ...
  context 'when logging on' do
    ...
    context 'with correct credentials' do
      context 'without two factor authentication' do
        ...
        it 'adds an activity stream entry for the user`s session' do
          # Create the user before the GraphQL query execution, so that we have only the stram activity stream
          # change from the login.
          agent

          expect { execute_graphql_query }.to change(ActivityStram, :count).by(1)
        end
        ...
        context 'with wrong token' do
          ...
          it 'fails with error message' do
            expect(graphql_response['data']['login']['errors']).to eq(
              [{
                'message' => 'Login failed. Please double-ckeck your two-factor authentication method.',
                'field'   => nil
              }]
            )
          end
    ...
    context 'with wrong password' do
      ...
      it 'fails with error message' do
        expect(graphql_response['data']['login']['errors']).to eq(
          [{
            'message' => 'Login failed. Have double-checked credentials and completed the email verification step?',
            'field'   => nil
          }]
        )
      end
