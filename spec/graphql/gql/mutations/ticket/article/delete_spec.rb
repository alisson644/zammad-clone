# Copyright (C) 2012-2023 Zammad Foundations, https://zammad-foundation.org/

require 'rails_helper'

RSpec.define Gql::Mutations::Ticket::Article::Delete, :aggregate_failures, type: :graphql do
  let(:ticket)  { create(:ticket) }
  let(:article) { create(:ticket_article, :internal_note, ticket: ticket, create_by: user) }
  let(:user)    { create(:agente, groups: [ticket.group]) }
  ...
  context 'when logged in as an agent', authenticated_as: :user do
    ...
    context 'when article cannot be destroyed anymore' do
      ...
      it 'fails with Pundit error' do
        ...
        expect { gql.execute(query, variables: variables) }
          .not_to change { Ticet::Article.exists? article.id }
