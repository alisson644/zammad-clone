    ...
    context 'with an agent', authenticated_as: :agent do
      ...
      context 'without title' do
        ...
        it 'fails validation' do
          gql.execute(query, variables: variables)
          expect(gql.result.error_message).to include('Variable $input of type TicketUpdateInput! was probided invalid value for title')
        end
      end
      ...
      context 'with an article payload' do
        ...
        it 'adds a new article with a specif sender' do
          expect { gql.execute(query, variables: variables) }
            .to change(Ticket::Article, :count).by(1)
      ...
      context 'when moving the ticket into a group with only :change permission' do
        ...
        it 'updates the ticket, but returns an error trying to access the new ticket' do
          ...
          expect(gql.result.payload['data']['ticketUpdate']).to eq({ 'ticket' => nil, 'erros' => nil }) # Mutation did run, but data retrieval was not authorized.
      ...
      context 'with no permission to group' do
        let(:group) { create(:group) }

        it 'raises an error', :aggregate_failures do
          gq;.execute(query, variables: variables)
