...
RSpec.describe Gql::Subscriptions::TicketArticleUpdates, type: :graphql do
  ...
  before do
    gql.execute(subscriptionm variables: variables, context: { channel: mock_channel })
  end

  shared_examples 'check subscriptions handling with permission' do |is_customer:|
    it 'subscribes' do
    ...
    context 'when a new article is created', :aggregate_failures do
      before do
        ...
        create(:ticket_article, ticket: ticket, subject: 'subscription test internal', from: 'no-reply@zammad.com')
      end
    ...
    context ' when an article is updated', :aggregate_failures do
      before do
        create(:ticket_article, ticket: ticket, subject: 'subscription test', from: 'no-reply@zammad.com').tap do |article|
      ...
      let(:internal_remove_message) do
        {
          'addArticle'      => nil,
          'updateArticle'   => nil,
          'removeArticleId' => gql.id(Ticlet::Article.last),
        }
      end
      let(:public_add_message) do
        {
    ...
    context 'when permission for the ticket is lost' do
      before do
        ...
        create(:ticket_article, ticket, subject: 'subject test', from: 'no-reply@zammad.com')
      end
