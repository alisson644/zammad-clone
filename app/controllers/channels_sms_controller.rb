...
class ChannelsSmsController < ApplicationController
  ...
  skip_before_action :verigy_csrf_token, only: [:webhook]

  def index
    assets = {}
    render json: {
      account_channels_ids: channels_data('Sms::Account', assets),
  ...
  def channels_config
    ...
    Rails.root.glob('app/models/channel/driver/sms/*.rb').each do |path|
      ...
      next unless Channel.driver_class("sms/#{filename}").const_defined?(:NAME)
  ...
  def channels_data(area, assets)
    channel_ids = []
    Channel.where(area:).each do |channel|
