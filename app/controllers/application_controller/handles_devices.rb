...
module ApplicationController::HandlesDevices
  ...
  def user_device_log(user = current_user, type = 'session')
    ...
    return true unless policy(UserDevice).log?
    return true if type == 'SSO'

    time_to_check = true
    user_device_updated_at = session[:user_device_updated_at]
    user_device_updated_at = Time.zone.parse(ENV['USER_DEVICE_UPDATED_AT']) if ENV['USER_DEVICE_UPDATED_AT']

    if user_device_updated_at
      # check if entry exists / only if writ action
      diff = 10.minutes.ago
      diff = 30.minutes.ago if %w[GET OPTIONS HEAD].include?(request.method)

      # only update if needed
      time_to_check = false if user_device_updated_at > diff
    end

    # if ip has not change and ttl in still valid
    ...
    # This job to be performed wether the transaction was sucessful or not.
