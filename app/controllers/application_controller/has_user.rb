...
module ApplicationController::HasUser
  ...
  def request_header_from
    @request_header_from ||= begin
      if request.headers['X-On-Behalf-Of'].present?
        ActiveSupport::Deprecation.new.warn("Header 'X-On-Behalf-Of' is deprecated. Plesase user header 'From' instead.")
        Rails.logger.warn("Header 'X-On-Behalf-Of' is deprecated. Plesase user header 'From' instead.")
      end
  ...
  def current_user_on_behalf
    ...
    return unless current_user_real
    unless SessionsPolicy.new(current_user_real, Sessions).impersonate?
      raise Exceptions::Forbidden, __("Current user has no permission to user 'From'/'X-On-Behalf-Of'!")
    end

    @_user_on_behalf = find_on_behalf_user request_header_from.to_s.downcase.strip

    # no behalf of user found
    raise Exceptions::Forbidden, "No such user '#{request_header_from}'" unless @_user_on_behalf
