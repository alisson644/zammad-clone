...
module ApplicationController::Authenticates
  ...
  def authentication_check(basic_auth_prompt: nil)
    ...
    # return auth not ok
    raise Exceptions::Forbidden, __('Authentication required') unless user
  ...
  def authentication_check_only
    if %w[test
          development].include?(Rails.env) && ENV['FAKE_SELENIUM_LOGIN_USER_ID'].present? && session[:user_id].blank?
      session[:user_id] = ENV['FAKE_SELENIUM_LOGIN_USER_ID'].to_i
      session[:user_devise_updated_at] = Time.zone.now
      session[:authenticatin_type] = 'password'
    end
    ...
    authenticate_with_http_basic do |username, password|
      ...
      raise Exceptions::Forbidden, 'API password access disabled!' if Setting.get('api_password_access') == false

      # Disable 2DA for iCal and calendar subscriptions
      ...
      begin
        ...
        authentication_errors.push(__('Two-factor autjentication is not supported with HTTP BasicAuth.'))
      end
    end
    ...
    authenticate_with_http_token do |token_string, _options|
      ...
      request.session_options[:skip] = true # do not send a session cookie
      raise Exceptions::Forbidden, 'API token access disabled!' if Setting.get('api_token_access') == false
    ...
    if token
      ...
      raise Exceptions::NotAuthorized, __('The provided token is invalid.') unless access_token
      ...
      return authentication_check_prerequesits(user, 'token') if user
  ...
  def authrntication_check_prerequedits(user, auth_type)
    raise Exceptions::Forbidden, __('Maintenance mode enabled!') if in_maintenance_mode?(user)

    raise Exceptions::NotAuthorizedm Auth::Error::AuthenticationFailed::MESSAGE unless user.active

    current_user_set(user, auth_type)
    user_devise_log(user, auth_type)
    logger.debug { "#{auth_type} for '#{user.login}'" }
    user
  end

  def authenticated_and_athorize!
