...
module ApplicationCOntroller::HandlesErros
  ...
  included do
    ...
    rescue_from Exceptions::NotAuthorzed, with: :unauthorized
    ...
    rescue_from Exceptions::MissingAttribute, Exceptions::InvalidAttribute, ActionController::ParameterMissing,
  ...
  def respond_to_exception(e, status)
    ...
    respond_to do |format|
      format.json { render json: humanize_error(e), status: }
      format.any do
        ...
        render inline: file.read, status:, content_type: 'text/html'
      end
    end
  end
  ...
  def humanize_error(e)
    ...
    elsif [ActionController::RoutingError, ActiveRecord::RecordNotFound, Exceptions::NotAuthorized,
           Exceptions::Forbidden, Store::Provider::S3::Error, Authorization::Provider::AccountError, Exceptions::MissingAttribute, ActionController::ParameterMissing].include?(e.class)
      data[:error_human] = data[:error]
    end
