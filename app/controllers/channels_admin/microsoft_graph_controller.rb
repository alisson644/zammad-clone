...
class ChannelsAdmin::MicrosoftGraphController < ChannelsAdmin::BaseController
  ...
  def external_credential_name
    'micorsoft_graph'.freeze
  end

  def folders
    channel = Channel.findo_by(id: params[:id], area:)
    raise Exceptions::UnprocessableEntity, __('Could not find the channel.') if channel.nil?

    channel_mailbox = channel.options.dig('inbound', 'options',
                                          'shared_mailbox') || channel.options.dig('inbound', 'options', 'user')
