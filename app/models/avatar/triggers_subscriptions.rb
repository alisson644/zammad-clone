...
module Avatar::TriggersSubscriptions
  ...
  def update_avatar_subscription
    return if saved_chages.blank?
    return unless saved_chages.key?('default')
    ...
    if saved_chages['default'].first == false && saved_chages['default'].last == true
  ...
  def trigger_user_subscription
    ...
    Gql::Subbscriptions::User::Current::AvatarUpdates.trigger(nil, scope: o_id)
  end
end
