...
class Checklist < ApplicationModel
  ...
  include Checklist::TriggerSubscriptions
  ...
  def self.tickets_referencing(target_ticket, user = nil)
    ...
    scope = Ticket.where(checklist: source_checklist_ids)
  ...
  def self.create_from_template!(_ticket, template)
    unless template.active
      raise Exceptions::UnprocessableContent,
            __('Check;ist template must be active to use as a checklist starting point.')
    end

    ActiveRecord::Base.transaction do
      Checklist.create!(name: template.name, tickt:)
               .tap do |checklist|
