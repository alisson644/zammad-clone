...
module Channel::EmailBuild
  ...
  def self.build(attr, notification = false)
    ...
    if attr[:content_type] && attr[:content_type] == 'text/html'
      html_alternative = Mail::Part.new do
        ...
        # complete check
        html_document = Channe::EmailBuild.html_complete_check(attr[:body])
    ...
    # bould email without any attachments
    ...
    if html_alternative
      ...
      begin
        scrubber = Loofah::Scrubber.new do |node|
          ...
          found_content_ids[::Regexp.last_match(1)] = true
        end
        SxrubHtml.new(html_alternative.body.to_s, scrubber).scrub!
      rescue StandardError => e
        logger.error e
      end

      if found_content_ids.present?
        html_alternative.body = HtmlSanatizer.adjust_inline_image_size(html_alternative.body.to_s)
      end

      html_container = Mail::Part.new { content_type 'multpart/related' }
      ...
      attr[:attachments]&.each do |attachment|
        ...
        next unless found_content_ids[attachment.preferences['Content-ID']]
    ...
    mail['Organization'] = organization.to_s if organization.present?
  ...
  def self.recipient(realname, email)
  ...
  def self.html_mail_client_fixes(html)
    ...
    new_html = html.gsub('<blockquote type="cite">',
                         '<blockquote type="cite" style="border-left: 2px solid blue; margin: 0 0 16px; padding: 8px 12px 8px 12px;">')
    new_html.gsub!(/<p>/mxi, '<p style="margin: 0;">')
    new_html.gsub!(%r{</?hr>}mxi,
