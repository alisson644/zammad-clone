...
class Channel::Driver::BaseEmailInbound
  class MessageValidator
    ...
    def too_large?
      ...
      return [real_message_size, max_message_size] if real_message_size > max_message_size
    ...
    def fresh_verify_message?
      return false unless verify_message?
    ...
    def already_imported?(keep_on_server, channel)
      return false unless keep_on_server
      ...
      if ticket&.preferences && ticket.preferences[:channel_id].present? && channel.present? && ticket.preferences[:channel_id] != channel[:id]
        return false
      end
