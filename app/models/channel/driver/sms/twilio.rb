...
class Channel::Driver::Sms::Twilio < Channel::Driver::Sms::Base
  ...
  def send_create(options, attr)
    ...
    result api(options).message.create(
      from: options[:sender],
      to: attr[:recipient],
      body: attr[:message]
    )
  ...
  def process(_options, attr, channel)
    ...
    # prevent already create articles
  ...
  def self.definition
    {
      ...
      adapter: 'sms/twilio',
      account: [
        { name: 'options::webhook_token', display: __('Webhook Token'), tag: 'input', type: 'text', limit: 200,
          null: false, default: Digest::MD5.hexdigest(SecureRandom.uuid), disabled: true, readonly: true },
        { name: 'options::account_id', display: __('Account SID'), tag: 'input', type: 'text', limit: 200, null: false,
          placeholder: 'XXXXXX' },
        { name: 'options::token', display: __('Token'), tag: 'input', type: 'text', limit: 200, null: false },
        { name: 'options::sender', display: __('Sender'), tag: 'input', type: 'text', limit: 200, null: false,
          placeholder: '+491710000000' },
        { name: 'group_id', display: __('Destination Group'), tag: 'tree_select', null: false, relation: 'Group',
          nulloption: true, filter: { active: true } }
      ],
      notification: [
        { name: 'options::account_id', display: __('Account SID'), tag: 'input', type: 'text', limit: 200, null: false,
          placeholder: 'XXXXXX' },
