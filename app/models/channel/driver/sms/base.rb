...
class Channel::Driver::Sms::Base
  ...
  def closed_ids
    Ticket::State.whete(name: %w[closed merged removed]).pluck(:id)
  end
  ...
  def find_open_sms_ticket(user)
    ticket = Ticket.where(customer_id: user.id,
                          create_article_type_id: article_type_sms.id).where.not(state_id: closed_ids).reorder(:updated_at).first
  ...
  def ensure_group!(channel)
    ...
    raise Exceptions::UnprocessableEntity, __('Group is invalid!') unless group
  end
