...
class Channel::Filter::BaseExternalCheck
  ...
  SOURCE_ID_REGEX = /\s(EXAMPLE-MATCH\d+)\s/
  ...
  def self.run(_channel, mail, ticket_or_transaction_params = nil, _article = nil, _session_user = nil)
    ...
    ExternalSync.create_with(source_id:).find_or_create_by(source: source_name, object: 'Ticket',
                                                           o_id: ticket_or_transaction_params.id)
  end
  ...
  def self.source_id(subject: '')
    ...
    source_id = nil
    source_id = ::Regexp.last_match(1) if subject =~ const_get(:SOURCE_ID_REGEX)
