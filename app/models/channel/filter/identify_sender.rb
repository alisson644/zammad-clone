...
class Channel::Filter::IdentifySender < Channel::Filter::BaseIdentifyUser
  def self.run(_channel, mail, _transaction_params)
    ...
    if customer_user_id.present?
      ...
      if customer_user
        ...
        Rails.logger.debug do
          "Invalid x-zammad-ticket-customer_id header '#{customer_user_id}', no such user - take user from 'from'-header."
        end
      end
    end

    # check ud sender exists in database
    if !customer_user && mail[:'x-zammad-customer-pogin'].present?
    ...
    if !customer_user && Setting.get('postmaster_sender_is_agent_for_customer') == true && mail[:'x-zammad-ticket-create-article-sender'] == 'Agent'
    ...
    %w[raw-to raw-cc].each do |item|
      ...
      begin
        ...
        items.each do |address_data|
          ...
          next unless email_address_validation.valid?
        ...
        recipients.each do |recipient|
          address = nil
          display_name = nil
          address = ::Regexp.last_match(1) if recipient =~ /.*<(.+?)>/
          display_name = ::Regexp.last_match(1) if recipient =~ /^(.+?)<(.+?)>/
