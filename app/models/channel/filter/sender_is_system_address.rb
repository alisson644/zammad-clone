...
module Channel::Filter::SenderIsSystemAddress
  def self.run(_channel, mail, _transaction_params)
    ...
    # in case, set sender
    begin
      return unless mail[form].addrs

      items = mail[form].addrs
      items.each do |item|
        next unless EmailAddress.exists?(email: item.address.downcase)
    ...
    return if mail[:from_mail].blank?
  ...
  def self.detect_article_sender(mail, user)
    ...
    return unless user.group_access?(group, :any)
    ...
    if ticket_id.present?
      ...
      mail[:'x-zammad-article-sender'] = 'Customer' if ticket.present? && ticket.customer_id == user.id
    end
  rescue StandardError => e
    Rails.logger.error "SenderIsSystemAddress: #{e.inspect}"
  end
  ...
  def self.detect_ticket_create_article_sender(mail, user)
    ...
    # if the agent is also customer of ticket then
    # we need to set the sender as customer.
    ticket_id = mail[:'x-zammad-ticket-id']
    if ticket_id.present?
      ticket = Ticket.lookup(id: ticket_id)

      mail[:'x-zammad-ticket-create-article-sender'] = 'Customer' if ticket.present? && ticket.customer_id == user.id
    end
  rescue StandardError => e
    Rails.logger.error "SenderIsSystemAddress: #{e.inspect}"
  end
