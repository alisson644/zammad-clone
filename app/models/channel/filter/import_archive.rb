...
module Channel::Filter::ImportArchive
  def self.run(channel, mail, transaction_params)
    return unless archivable?(channel, mail)
    ...
    # disable notification and trigger
  ...
  def self.archivable?(channel, mail)
    return false unless mail[:date]

    options = channel_options(channel)
    return false if options[:archive] != true
    return false unless archiable_date_range?(channel, mail)

    true
  end

  def self.archiable_date_range?(channel, mail)
    options = channel_options(channel)
  ...
  def self.overwrite_created_at(mail)
    mail[:'x-zammad-ticket-create_at'] = mail[:date]
