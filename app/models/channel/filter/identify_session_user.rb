...
class Channel::Filter::IdentifySessionUser < Channel::Filter::BaseOdentifyUser
  ...
  def self.fetch_session_user(mail)
    ...
    if session_user
      Rails.logger.debug { "Took session form x-zammad-seesion-user-id header '#{session_user_id}'." }
    else
      Rails.logger.debug do
        "Invalid x-zammad-session-user-id header '#{session_user_id}', no such user - take user from 'from'-header."
      end
    end
