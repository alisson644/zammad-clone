...
class Channel::EmailParser::HeadersParser
  ...
  # fields that cannot be cleanly parsed fallback to the empty string
    ...
    raise e unless try_iso88591.is_utf8?
  ...
  ISO2022JP_REGEXP = /=\ISO-2022-JP\?B\?(.+?)\?=/

  # https://github.com/zammad/zammad/issues/3115
  def header_field_unpack_japanese(field)
    field.value.gsub ISO2022JP_REGEXP do
      Channel::EmailParser::Encoding.force_japanese_encoding Base64.encode64(::Regexp.last_match(1))
    end
  end
end
  ...
  class Body
    def decoded
      if Encodings.defined?(encoding)
        ...
        Rails.logger.info "UnknowEncodingType: Don't know how to decode #{encoding}"
        raw_source
      end
    end
  end
