...
class Channel::EmailParser::MessageWalker
  ...
  def process_message_body
    ...
    walk_message_parts(mail) do |part, style|
      ...
      content_type = local_content_type if local_content_type == 'text/html' || content_type.blank?
  ...
  def process_single_message_part(_part, style:, raw_html:)
  ...
  def walk_single_part(part, force_html:, &)
    ...
    if part.attachments?
      yield part, :inline_image if part_inline_image?(part)
  ...
  def walk_multi_part(part, force_html:, &)
    cotent_type = part.content_type

    if cotent_type.start_with?('multipart/mixed')
  ...
  def walked_miltipart_related(part, force_html:, &)
    if part.html_part.present?
      yield part.html_part, :html
    elsif part.text_part.present?
      has_inline_attachment = part.parts.any? { |p| p.attachment? && part_inline_image?(p) }
  ...
  def body_text(message, **options)
    ...
    return [body_text, {}] unless options[:strict_html]

    # Issue #2390 - email with > 5k HTML links should be rejected
    return [Channel::EmailParser::EXCESSIVE_LINKS_MSG, {}] if body_text.scan(/<a[[:space:]]/i).count >= 5_000

    body_text.html2html_strict
  end

  def part_inline_image?(part)
    return false unless part.is_a?(Mail::Part)
