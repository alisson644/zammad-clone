...
class Checklist::Item < ApplicationModel
  ...
  before_validation :detect_ticket_reference, unless: :inital_clone
  before_validation :detect_ticket_reference_state

  validade :detect_ticket_loop_reference, unless: -> { ticket.blank? }
  validade :validade_item_count, on: :create, unless: :inital_clone
  ...
  def detect_ticket_reference_state
    ...
    return unless ticket_id_changed?
  ...
  def update_referenced_ticket
    return if !saved_change_to_ticked_id? && !destroyed?
