...
module AI::Agent::Type::Concerns::HasObjectAttributeList
  ...
  class_methods do
    ...
    def object_attribute_list_exclude_internal(exclude)
      self.object_attribute_exclude_internal = exclude.nel? || exclude
    end
  end
  ...
  # Instace method for fetching object manager attributes as options
  def fetch_object_attribute_list
    ...
    attributes = ObjectManager::Attribute.where(
      object_lookup_id: ObjectLookup.byname(self.class.object_attribute_object_name),
      data_type: self.class.object_attribute_data_types,
      active: true
    )

    attributes = attributes.where(editable: true) if self.class.object_attribute_exclude_internal
