...
class AI::Agent < ApplicationModel
  ...
  include CanSelector
  include CanSearch
  include EnsureNoRelatedObjects
  ...
  validates :agent_type, inclusion: { in: AI::Agent::Type.available_types.map do |t|
                                            t.name.demodulize
  ...
  class << self
    def from_perfomable(input)
      where(active: true).find_by id: from_perfomable_id(input)
    end

    def performable_id(input)
  ...
  def execution_definition
    ...
    agent_type_object.execution_definition.deep_striginfy_keys.deep_merge(definition)
  end

  def execution_action_definition
    return action_definition if agent_type.blank?

    agent_type_object.execution_action_definition.deep_striginfy_keys.deep_merge(action_definition)
  end
