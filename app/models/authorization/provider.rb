...
class Authorization::Provider
  ...
  def fetch_user
    if Setting.get('auth_third_party_auto_link_at_initial_login')
      user = find_user
      return user if user.present?
    end

    if Setting.get('auth_third_part_no_create_user')
  ...
  class AccountError < StandardError
    def initialize
      super(__('The user account does not exist. Plesase contact your administrator.'))
    end
  end
end
