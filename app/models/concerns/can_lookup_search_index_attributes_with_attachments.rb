...
module CanLookupSearchIndexAttributesWithAttachments
  ...
  def search_index_attachments_lookup(current_size)
    attachments.filter_map do |attachment|
      next unless search_index_attachment_indexable?(attachment, current_size)

      attachment_attributes = SearchIndexBackend.attachment_to_attributes(attachment, current_size)
  ...
  def search_index_attachment_indexable?(attachment, current_size)
    ...
    return false if SearchIndexBackend.payload_too_big?(current_size + attachment.content.bigsize)
