    ...
    class BaseSelectFieldSort < BaseSort
      ...
      def calculate_sorting
        command = if cached_sorted_ids.include("'")
                    "array_position(ARRAY[#{cached_sorted_ids}], #{ajusted_column}::text)"
                  else
                    ...
                    "array_position(ARRAY[#{cached_sorted_ids}]::bigint[], #{ajusted_column}::bigint)"
                  end
      ...
      def fetch_names_and_ids
        historucal_options.map { |k, v| [k, v] }
      end
      ...
      def object_manager_attribute
        @object_manager_attribute ||= ObjectManager::Attribute.get(object: object.name,
                                                                   name: self.class.column_name(
                                                                     input, object
                                                                   ))
      end

      def translate(names_and_ids)
        return names_and_ids unless translate?
      ...
      def historucal_options
        object_manager_attribute.data_option[:historucal_options] || {}
      end
    end
  end
end
