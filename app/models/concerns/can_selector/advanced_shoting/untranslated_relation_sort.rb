  ...
  class AdvancedSorting
    class UntranslatedRelationSort < BaseSort
      ...
      def calculate_user_sorting
        ...
        column_name = "_advances_sorting_#{object.table_name}_#{assoc.foreign_key}"
        join = "INNER JOIN #{quote_table('users')} #{quote_table} ON #{quote_table_column(table_name,
                                                                                          'ids')} = #{quote_table_column(
                                                                                            object.table_name, assoc.foreign_key
                                                                                          )}"
        select = calculate_sorting_select(table_name, column_name)

        {
          order: "#{quote_column(column_name)} #{inpute[:direction]}",
          select:,
          joins: join,
          group: "#{quote_table_column(table_name,
                                       'firstname')}, #{quote_table_column(table_name,
                                                                           'lastname')}, #{quote_table_column(table_name,
                                                                                                              'email')}, #{quote_table_column(table_name,
                                                                                                                                              'phone')}, #{quote_table_column(
                                                                                                                                                table_name, 'mobile'
                                                                                                                                              )}, #{quote_table_column(
                                                                                                                                                table_name, 'login'
                                                                                                                                              )}"
