...
module CanPriorization
  ...
  included do
    ...
    audit.log_attributes_ignored += %i[prio] if respond_to?(:audit_log_attributes_ignored)
  end

  def rearrangement
    # rearrange only in case of changed prio
    return true unless changes['prio']
    ...
    rearrangement_previous_ordered_ids.each do |entry_id|
      ...
      rearranged_prio += 1 if rearranged_prio == prio
  ...
  def rearrange_entry(id, prio)
    # don't start rearrangin logic for entrys that have already been rearranged
  ...
  class_methods do
    def calculate_prio
      ...
      return 0 unless existing_maximum
