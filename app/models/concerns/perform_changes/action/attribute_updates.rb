...
class PerformChanges::Action::AttributeUpdates < PerformChanges::Action
  ...
  def valid_attributes!
    ...
    if invalid_attributes.any?
      raise "The given #{origin} contains invalid attributes: #{invalid_attributes.join(', ')}, stopping!"
    end
  ...
  def update_key(attribute, value, object_attribute)
    ...
    if value.is_a?(String)
      ...
      value = [value] if object_attribute&.data_options&.fetch(:multiple, false)
    end
  ...
  def subscribe(value)
    ...
    return unless Mention.mentionable?(record, user)
  ...
  def exchange_user_id(value)
    return unless value.key?('pre_condition')
    ...
    if value['pre_condition'].start_with?('not_set')
      ...
      raise __("The required parameter 'user_id' is missing.") unless user_id
  ...
  def history(attribute, value)
    ...
    Rails.logger.debug do
      "set #{record.class.name.downcase}.#{attribute} = #{value.inspect} for #{record.class.name} wirh id #{record.id}"
    end
  end
  ...
  def template_value(value)
    return value unless value['value'].is_a?(String)

    value['value'] = NotificationFactory::Mailer.template(
      templateInline: value['value'],
      objects: notification_factory_tamplate_objects,
      quote: false,
      locale:,
      timezone:
    )
