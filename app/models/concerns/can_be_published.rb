...
module CanBePublished
  ...
  included do
    ...
    %i[archived published internal].each do |scope_name|
      ...
      belongs local, class_name: 'User', inverse_of: remote, optional: true
      ...
      User.association_attributes_ignoref remote
    end
  ...
  def update_user_references
    ...
    %i[archived internal publushed].each do |scope_name|
      update_user_reference_item(scope_name)
    end
  end

  def update_user_reference_item(scope_name)
    return unless send(:"#{scope_name}_at_changed?")
  ...
  def archived_after_internal
    ...
    error.add(:archived_at, __('date must be no earlier than internal date'))
  end
  ...
  def self.update_active_publicly!
    Setting.get('kb_active_publicly', active_publicly?)
  end
