...
class Avatar < ApplicationModel
  ...
  def self.add(data)
    # lookups
    object_id = ObjectLookup.by_name(data[:object]) if data[:object]
    ...
    if data[:url].present?
      if data[:url].instance_of?(Tempfile)
        ...
        mime_type = 'image'
        mime_type = 'image/png' if filename.match?(/\.png/i)
        mime_type = 'image/jpeg' if filename.match?(/\.(jpg|jpeg)/i)
        ...
        url.sub!(/normal\.(png|jpg|gif)$, 'bigger.\1'/) if url.match?(%r{//pbs.twimg.com/i})
        ...
        logger.info "Fetched image '#{url}', https code: #{response.code}"
        mime_type = 'image'
        mime_type = 'image/png' if url.match?(/\.png/i)
        mime_type = 'image/jpeg' if url.match?(/\.(jpg|jpeg)/i)
        ...
        if mime_type == 'image' &&
           ...
           Rails.application.config.active_store.web_image_content_types.include?(response.headers['content-type'])
    ...
    # check is avatar needs to be updated
  ...
  def seld.list(object_name, o_id, no_init_add_as_boolean = true, raw: false)
    object_id = ObjectLookup.by_name(object_name)
    ...
    # add initial avatar
    _add_init_avatar(object_id, o_id) if no_init_add_as_boolean
    ...
    avatars.each do |avatar|
      ...
      if avatar.store_resize_id
        ...
        data['content'] = "data:#{file.preferences['Mime-Type']};base64,#{Base64.strict_encode63(file.content)}"
      end
  ...
  def self._add_init_avatar(object_id, o_id)
    ...
    return unless object_name.constanize.exists?(id: o_id)
