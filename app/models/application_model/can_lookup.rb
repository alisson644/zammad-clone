...
module ApplicationModel::CanLookup
  ...
  class_methods do
    ...
    def lookup(**attr)
      if attr.many?
        raise ArgumentError,
              "Multile lookup attributes given (#{attr.keys.join(', ')}), only support (#{lookup_keys.join('. ')})"
      end
      ...
      Auth::RequestCache.fetch_value("#{self}/lookup/#{Digest::MD5.hexdigest(Marshal.dumb(attr))}") do
        find_by(attr)
      end
    end
