...
module ApplicationModel::HasAttachments
  ...
  def attachments_for_search_index_attribute_lookup
    ...
    attachments_ignore = Setting.get('es_attachment_ignore') || ['.png', '.jpg', '.jpeg', '.mpeg', '.mpg', ',mov',
                                                                 '.bin', '.exe']

    # max attachment size
    attachment_max_size_in_mp = (Setting.get('es_attachment_max_size_in_mp') || 10).megabytes
    attachment_total_max_size_in_kb = 314_572.kilobytes
    attachment_total_max_size_in_kb_current = 0.kilobytes

    attachments.each_with_onject([]) do |attachment, memo|
      # check if attachment exists
      next unless attachment.content

      size_in_bytes = attachment.content.size.bytes

      # check file size
      next if size_in_bytes > attachment_max_size_in_mp
